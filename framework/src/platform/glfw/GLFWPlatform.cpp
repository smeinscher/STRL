...
std::map<int, unsigned int> glfw_key_to_strl_key = {KEY_MAP_ITEM(LEFT_CONTROL),
...
std::map<int, int> glfw_mouse_button_to_strl_mouse_button = {
    {GLFW_MOUSE_BUTTON_1, STRL_MOUSE_BUTTON_0},          {GLFW_MOUSE_BUTTON_2, STRL_MOUSE_BUTTON_1},
    {GLFW_MOUSE_BUTTON_3, STRL_MOUSE_BUTTON_2},          {GLFW_MOUSE_BUTTON_4, STRL_MOUSE_BUTTON_3},
    {GLFW_MOUSE_BUTTON_5, STRL_MOUSE_BUTTON_4},          {GLFW_MOUSE_BUTTON_6, STRL_MOUSE_BUTTON_5},
    {GLFW_MOUSE_BUTTON_7, STRL_MOUSE_BUTTON_6},          {GLFW_MOUSE_BUTTON_8, STRL_MOUSE_BUTTON_7},
    {GLFW_MOUSE_BUTTON_LEFT, STRL_MOUSE_BUTTON_LEFT},    {GLFW_MOUSE_BUTTON_RIGHT, STRL_MOUSE_BUTTON_RIGHT},
...
GLFWPlatform::GLFWPlatform(int window_width, int window_height, bool fullscreen, std::string window_name)
    : window_width_(window_width), window_height_(window_height), fullscreen_(fullscreen),
...
void GLFWPlatform::toggle_fullscreen()
{
    ...
    if (monitor)
    {
        ...
        glfwSetWindowMonitor(window_, NULL, 0, 0, window_width_, window_height_, 0);
    }
    else
    {
...
void GLFWPlatform::update_window()
{
    GLFWmonitor *monitor = glfwGetWindowMonitor(window_);
    if (!monitor)
    {
...
{
    window_ = glfwCreateWindow(window_width_, window_height_, window_name_.c_str(),
...
void GLFWPlatform::setup_glfw_callbacks()
{
    ...
    glfwSetKeyCallback(window_, [](GLFWwindow *window, int key, int scancode, int action, int mods) {
        ...
        STRLEvent *event = reinterpret_cast<GLFWPlatform *>(glfwGetWindowUserPointer(window))
                               ->get_event_manager()
                               .get_by_event_code(event_type, glfw_key_to_strl_key[key]);
        if (event)
        {
    ...
    glfwSetScrollCallback(window_, [](GLFWwindow *window, double xoffset, double yoffset) {
        STRLEvent *event = reinterpret_cast<GLFWPlatform *>(glfwGetWindowUserPointer(window))
                               ->get_event_manager()
                               .get_by_event_code(STRLEventType::STRL_EVENT_MOUSE_WHEEL, 0);
        if (event)
        {
    ...
    glfwSetMouseButtonCallback(window_, [](GLFWwindow *window, int button, int action, int mods) {
        ...
        STRLEvent *event = reinterpret_cast<GLFWPlatform *>(glfwGetWindowUserPointer(window))
                               ->get_event_manager()
                               .get_by_event_code(event_type, glfw_mouse_button_to_strl_mouse_button[button]);
        if (event)
        {
            event->fire_event(&mods);
        }
    ...
    glfwSetCursorPosCallback(window_, [](GLFWwindow *window, double xpos, double ypos) {
        STRLEvent *event = reinterpret_cast<GLFWPlatform *>(glfwGetWindowUserPointer(window))
                               ->get_event_manager()
                               .get_by_event_code(STRLEventType::STRL_EVENT_MOUSE_MOVED, 0);
        if (event)
        {
