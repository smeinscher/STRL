...
STRLObject::STRLObject(STRLObjectDefinition &definition)
    : STRLManagedItemBase(definition.name.empty() ? "My Object" : definition.name, definition.tags),
      points_(definition.points), object_type_(definition.object_type), position_(definition.position),
...
{
    STRLObjectMessage message = {STRLObjectMessage::STRLObjectUpdateType::POINTS, this, point_change_count};
    observer_subject_.notify(message);
}
