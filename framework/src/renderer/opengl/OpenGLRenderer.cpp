...
void OpenGLRenderer::enable_depth_test(bool b)
{
    if (b)
    {
        glEnable(GL_DEPTH_TEST);
...
template <VDType... TYPES> void OpenGLRenderer::setup_render_data(OpenGLRenderData &render_data, TYPES... args)
{
    if (!platform_exists_)
    {
        return;
...
void OpenGLRenderer::update_vertex_data(OpenGLRenderData &render_data, VertexDataType type)
{
    ...
    if (!vertices.empty())
    {
        ...
        {
            glBufferSubData(GL_ARRAY_BUFFER, 0, static_cast<GLsizeiptr>(vertices.size() * sizeof(float)),
                            vertices.data());
        }
        else
        {
...
void OpenGLRenderer::update_index_data(OpenGLRenderData &render_data)
{
    ...
    if (!indices.empty())
    {
        ...
        {
            glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, static_cast<GLsizeiptr>(indices.size() * sizeof(int)),
                            indices.data());
        }
        else
        {
...
void OpenGLRenderer::setup_vertices(OpenGLRenderData &render_data)
{
    ...
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, render_data.get_ebo());
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, render_data.get_indices().size(), render_data.get_indices().data(),
                 GL_DYNAMIC_DRAW);
