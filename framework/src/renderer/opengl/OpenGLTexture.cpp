...
OpenGLTexture::OpenGLTexture()
    : internal_format_(GL_RGBA), image_format_(GL_RGBA), wrap_s_(GL_REPEAT), wrap_t_(GL_REPEAT), filter_min_(GL_LINEAR),
      filter_max_(GL_LINEAR)
{
}

OpenGLTexture::OpenGLTexture(GLint internal_format, GLint image_format, GLint wrap_s, GLint wrap_t, GLint filter_min,
                             GLint filter_max)
    : internal_format_(internal_format), image_format_(image_format), wrap_s_(wrap_s), wrap_t_(wrap_t),
...
bool OpenGLTexture::generate(const std::string &path)
{
    ...
    int nr_channels;
    unsigned char *data = stbi_load(path.c_str(), &width_, &height_, &nr_channels, STBI_rgb_alpha);
    if (!data)
    {
        // TODO: logging stuff
