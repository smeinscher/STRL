...
b2BodyDef Box2DPhysics::generate_b2BodyDef(STRLObject *object, Box2DBodyData *data)
{
    b2BodyDef body_definition;
    body_definition.type = b2_dynamicBody;
    body_definition.position.Set(object->get_position().x * PHYSICS_SCALE, object->get_position().y * PHYSICS_SCALE);
    body_definition.angle = object->get_rotation().euler.z;
    body_definition.linearDamping = 2.0f;
    body_definition.angularDamping = 0.9f;
    if (data)
    {
        body_definition.userData.pointer = reinterpret_cast<uintptr_t>(data);
    }
...
std::unique_ptr<b2Shape> Box2DPhysics::generate_b2Shape(STRLObject *object)
{
    ...
    if (points.size() > 8)
    {
        ...
        circle_shape->m_radius = glm::length(points[0] - points[points.size() / 2]) / 2.0f * PHYSICS_SCALE;
        return circle_shape;
    }
...
b2FixtureDef Box2DPhysics::generate_b2FixtureDef(b2Shape *shape)
{
    ...
    fixture_definition.density = 50.0f;
    fixture_definition.friction = 1.8f;
