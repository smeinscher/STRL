...
void Formation::on_update()
{
    ...
    if (are_units_in_position_ && formation_progress_to_goal_ < 1.0f &&
        glm::length(goal_positions_[0] - start_position_) > glm::epsilon<float>())
    {
        formation_progress_to_goal_ += ROTATION_SPEED / glm::length(goal_positions_[0] - start_position_);
        current_formation_position_ =
            calculate_position(goal_positions_[0], start_position_, formation_progress_to_goal_);
...
{
    auto pred = [unit](const UnitInfo &u) { return unit == u.unit; };
...
void Formation::move_units_to_formation_position()
{
    ...
    for (UnitInfo &unit : units_)
    {
        if (unit.progress_to_formation < 1.0f)
        {
            ...
            unit.unit->get_object()->set_position(
                calculate_position(unit.goal_position, unit.start_position, unit.progress_to_formation));
        }
    }
}
    ...
    while (!unplaced_units.empty())
    {
        for (glm::vec3 &position : lead_positions)
        {
            ...
            for (UnitInfo &unit : unplaced_units)
            {
                ...
                auto pred = [unit](const UnitInfo &unit_info) { return unit.unit == unit_info.unit; };
            ...
            auto pred = [closest_unit](const UnitInfo &unit_info) { return closest_unit.unit == unit_info.unit; };
            if (!setting_goal)
            {
...
std::vector<glm::vec3> Formation::get_lead_positions(bool setting_goal)
{
    ...
    {
        glm::vec3 projected_position =
            calculate_position(goal_positions_[0], units_[0].unit->get_object()->get_position(), 0.01f);
    ...
    case FormationType::LINE_ACROSS:
        ...
        {
            glm::vec3 offset = right * units_[0].unit->get_object()->get_size().x *
                               std::floor(static_cast<float>(i + 1) / 2.0f) * 2.0f;
            if (i % 2 == 1)
            {
...
glm::vec3 Formation::calculate_position(glm::vec3 goal_position, glm::vec3 start_position, float distance_to_goal)
{
    ...
    glm::vec3 position =
        glm::rotate(rotation, current) * glm::length(goal_position - planet_->get_position()) + planet_->get_position();
